    ...
    for sub in subs:
        ...
        for cond in conditions:
            ...
            for r in range(len(rois)):
                ...
                avg_d_prime, avg_accuracy, individual_results = perform_scene_classification(
                ...
                df.loc[len(df)] = [
                    ...
                    avg_accuracy,
                    individual_results,
                    len(individual_results)  # Number of scene pairs that contributed to the average
                ]
...
def run_rsa(preproc_dir, sub, roi, nruns, conds, hpc, vis, pos, norm, exclusion_path, apply_exclusion):
    ...
    # Get the RSA matrix
    rsa_corrs = get_rsa_matrix(conds, condlist, masked_betas, nruns)
...
def run_rsa_analysis(rois, rois_bool, pos, subs, nruns, norm, fisher, user,
                     vis, preproc_dir, conditions, df, analysis, results_path, exclusion_path, apply_exclusion, save_matrix):
    # Loop over subjects
    for sub in subs:
        # Loop over conditions
        for cond in conditions:
...
if __name__ == "__main__":
    ...
    rois = ["Temporal Pole", "Heschl's Gyrus (includes H1 and H2)", "Occipital Pole",
            ...
            "HPC", "CA1", "CA2+3", "DG", "EC", "PHC", "PRC", "Subiculum", "HPC", "HPC"]  # Has to be the name the file is saved as

    rois_bool = [False, False, False, False, False, True,
                 True, True, True, True, True, True, True, True, True]  # Boolean to indicate if the program should look in ASHS
    ...
    #subs = ["02", "03", "13", "15", "16", "20", "21", "22", "26", "29", "30"]
    ...
    conditions = ['V-V', 'V-IC_V']  # Always use a - to mark a new condition, has to be a list of interactions.
    ...
    df_classification = pd.DataFrame(columns=[
        'sub', 'cond', 'roi', 'd_prime', 'accuracy', 'individual results','num_scene_pairs'
    ])
    ...
    results_path = f"/gpfs/milgram/scratch60/turk-browne/aa2842/sandbox/RSA_structs/subs_{len(subs)}_fisher_{fisher}_norm_{norm}_exclusion_{apply_exclusion}_savematrix_{save_matrix}_visual_RSA_Results"
    results_path = results_path + (".pkl" if save_matrix else ".csv")
    
    # Path to store results
    classification_results_path = f"/gpfs/milgram/scratch60/turk-browne/aa2842/sandbox/RSA_structs/scene_classification_subs_{len(subs)}_fisher_{fisher}_norm_{norm}_exclusion_{apply_exclusion}_Results.csv"
    ...
    if execute: df = run_rsa_analysis(rois=rois, rois_bool=rois_bool, pos=pos, subs=subs,
                                      ...
                                      df=df, analysis=analysis, results_path=results_path, exclusion_path = exclusion_path, apply_exclusion=apply_exclusion, save_matrix=save_matrix)
    
    # Run the classification analysis
    if execute and classify: df_classification = run_scene_classification_analysis(
